...
export const fromDocuments = async () => {
  const loader = new DirectoryLoader(resolve(process.cwd(), '1_qna/docs'), {
  ...
  await store.save(resolve(process.cwd(), '1_qna/docs'));
  return store;
};

export const fromExistingIndex = async () => {
  const store = await FaissStore.load('1_qna/docs', new OpenAIEmbeddings());
  return store;
}